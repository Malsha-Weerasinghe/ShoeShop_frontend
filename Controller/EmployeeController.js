...
$('#employeeSaveBtn').on('click', ()=>{
    ...
    var empCode = generateNextEmployeeId();

    // Set generated customerId to the customerIdTxt field
    $('#employeeCodeTxt').val(empCode);
   ...
   /!* var empCode = $('#employeeCodeTxt').val();*!/
    ...
    if (validate(empName, "Employee Name") && validate(gender, "Gender") && validate(status, "Status") && validate(designation, "Designation") && validate(role, "Role") && validate(dob, "Date of Birth") && validate(jd, "Join Date") && validate(branch, "Branch") && validate(addr1, "Address Line 1") && validate(addr2, "Address Line 2") && validate(addr3, "Address Line 3") && validate(contact, "Contact Number") && validate(mail, "Email") && validate(gn, "Guardian Name") && validate(emrCont, "Emergency Contact Number") && validate(image,"Employee Image")) {
...
function getLastEmployeeId() {
    ...
    return 'EMP00_000'; // Placeholder value, replace this with actual implementation
}*/
...
$("#employee_Table").on("click","tr", function (){
    let id = $(this).find("th");
    let data = $(this).find("td");

    $("#employeeCodeTxt").val(id.eq(0).text());
    $("#employeeNameTxt").val(data.eq(0).text());
    $("#employeeGenderOption").val(data.eq(1).text());
    $("#empStatusOption").val(data.eq(2).text());
    $("#designationTxt").val(data.eq(3).text());
    $("#roleOption").val(data.eq(4).text());
    $("#empBirthday").val(data.eq(5).text());
    $("#empJoinDate").val(data.eq(6).text());
    $("#branchOption").val(data.eq(7).text());
    $("#address1Txt").val(data.eq(8).text());
   /* $("#address2Txt").val(data.eq(9).text());
    $("#address3Txt").val(data.eq(10).text());*/
    $("#contactTxt").val(data.eq(9).text());
    $("#mailTxt").val(data.eq(10).text());
    $("#guardiaNameTxt").val(data.eq(11).text());
    $("#emgContactTxt").val(data.eq(12).text());
